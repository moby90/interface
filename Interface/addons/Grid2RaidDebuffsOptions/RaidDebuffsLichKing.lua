local RDDB = Grid2Options:GetRaidDebuffsTable()
RDDB["The Lich King"] = {
	[631] = { -- Ciudadela de la Corona de Hielo
		{ id = 758, name = "Icecrown Citadel" },
		["Lord Marrowgar"] = {
		order = 1, ejid = 1624,
		69065, -- Empalado
		},
		["Lady Deathwhisper"] = {
		order = 2, ejid = 1625,
		71289, -- Subyugar mente
		71204, -- Toque de insignificancia
		71237, -- Maldicion de sopor
		},
		["Gunship Battle"] = {
		order = 3, ejid = 1626,
		69651, -- Golpe hiriente
		},
		["Deathbringer Saurfang"] = {
		order = 4, ejid = 1628,
		72293, -- Marca del campeon caido
		72769, -- Aroma de sangre
		},
		["Festergut"] = {
		order = 5, ejid = 1629,
		69290, -- Esporas contagiadas
		69248, -- Gas inmundo
		72219, -- Hinchazon gastrica
		69278, -- Espora de gas
		},
		["Rotface"] = {
		order = 6, ejid = 1630,
		69674, -- Infeccion mutada
		69508, -- Pulverizador de babas
		30494, -- Moco pegajoso
		},
		["Professor Putricide"] = {
		order = 7, ejid = 1631,
		70215, -- Hinchazon gaseosa
		72454, -- Peste mutada
		70341, -- Charco de baba
		70342, -- Charco de baba
		70911, -- Peste desatada
		69774, -- Moco pegajoso
		},
		["Blood Prince Council"] = {
		order = 8, ejid = 1632,
		72999, -- Prisi�n de las Sombras
		71807, -- Chispas relumbrantes
		},
		["Blood-Queen Lana'thel"] = {
		order = 9, ejid = 1633,
		70838, -- Espejo de sangre
		71623, -- Tajo delirante
		70949, -- Esencia de la Reina de Sangre
		72151, -- Sed de sangre frenetica
		71340, -- Pacto de los Caudo Oscuro
		72985, -- Sombras enjambradoras
		70923, -- Frenesi incontrolable
		},
		["Valithria Dreamwalker"] = {
		order = 10, ejid = 1634,
		70873, -- Vigor esmeralda
		70744, -- Rafaga de acido
		70751, -- Corrosion
		70633, -- Pulverizador de tripas
		71941, -- Pesadillas retorcidas
		70766, -- Estado onirico
		},
		["Sindragosa"] = {
		order = 11, ejid = 1635,
		70107, -- Escalofrio penetrante
		70106, -- Helado hasta los huesos
		69766, -- Inestabilidad
		71665, -- Asfixia
		70126, -- Senal de Escarcha
		70157, -- Tumba de hielo
		},
		["Lich King"] = {
		order = 12, ejid = 1636,
		72133, -- Dolor y sufrimiento
		68981, -- Invierno sin remordimientos
		69242, -- Chillido de alma
		69409, -- Segador de almas
		70541, -- Infestar
		27177, -- Profanar
		68980, -- Recolectar alma
		},
		["Trash"] = {
		order = nil, ejid = nil,
		70980, -- Trampa aracnida
		70450, -- Espejo de sangre
		71089, -- Pus burbujeante
		69483, -- Expiacion oscura
		71163, -- Devorar humanoide
		71127, -- Herida mortal
		70435, -- Descuartizar
		70671, -- Putrefaccion parasitante
		70432, -- Porrazo de sangre
		71257, -- Golpe barbarico
		},
	},
	[724] = { -- El Sagrario Rubi
		{ id = 761, name = "The Ruby Sanctum" },
		["Halion"] = {
		order = 1, ejid = 1652,
		74562, -- Combustion ignea
		74567, -- Marca de combustion
		74792, -- Consumo de alma
		74795, -- Marca de Consumo
		},
		["General Zarithrian"] = {
		order = nil, ejid = nil,
		74367, -- Rajar armadura
		},
		["Baltharus the Warborn"] = {
		order = nil, ejid = nil,
		74502, -- Marca enervante
		},
		["Saviana Ragefire"] = {
		order = nil, ejid = nil,
		74452, -- Conflagracion
		},
	},
	[616] = { -- El Ojo de la Eternidad
		{ id = 756, name = "The Eye of Eternity" },
		["Malygos"] = {
		order = nil, ejid = 1617,
		56272, -- Aliento Arcano
		57407, -- Oleada de poder
		},
	},
	[649] = { -- Prueba del Cruzado
		{ id = 757, name = "Trial of the Crusader" },
		["Lord Jaraxxus"] = {
		order = 2, ejid = 1619,
		66532, -- Bola de Fuego vil
		66237, -- Incinerar carne
		66242, -- Inferno ardiente
		66197, -- Llama de la Legion
		66283, -- Punta de dolor giratoria
		66209, -- Toque de Jaraxxus
		66211, -- Maldicion del infierno
		66333, -- Perforador nerubiano
		},
		["Faction Champions"] = {
		order = 3, ejid = 1621,
		65812, -- Afliccion inestable
		},
		["The Twin Val'kyr"] = {
		order = 4, ejid = 1622,
		},
		["Anub'arak"] = {
		order = 5, ejid = 1623,
		67574, -- Perseguido por Anub'arak
		66013, -- Frio penetrante
		66012, -- Tajo congelante
		},
		["Acidmaw"] = {
		order = nil, ejid = 1618,
		66819, -- Vomito acidulo
		66821, -- Vomito de arrabio
		66823, -- Toxina paralizadora
		66869, -- Bilis ardiente
		},
		["Gormok the Impaler"] = {
		order = nil, ejid = 1618,
		66331, -- Empalar
		66406, -- Snoboldado!
		},
		["Icehowl"] = {
		order = nil, ejid = 1618,
		66770, -- Golpe atroz
		66689, -- Aliento artico
		66683, -- Colision monumental
		},
	},
	[615] = { -- El Sagrario Obsidiana
		{ id = 755, name = "The Obsidian Sanctum" },
		["Sartharion"] = {
		order = 1, ejid = 1616,
		60708, -- Debilitar armadura
		57491, -- Tsunami de llamas
		},
		["Trash"] = {
		order = nil, ejid = nil,
		39647, -- Maldicion de alivio
		58936, -- Lluvia de Fuego
		},
	},
	[624] = { -- La Camara de Archavon
		{ id = 753, name = "Vault of Archavon" },
		["Archavon"] = {
		order = 1, ejid = 1597,
		},
		["Emalon"] = {
		order = 2, ejid = 1598,
		},
		["Koralon"] = {
		order = 3, ejid = 1599,
		},
		["Toravon the Ice Watcher"] = {
		order = 4, ejid = 1600,
		72004, -- Congelamiento
		},
	},
	[533] = { -- Naxxramas
		{ id = 754, name = "Naxxramas" },
		["Anub'Rekhan"] = {
		order = 1, ejid = 1601,
		28786, -- Enjambre de langostas
		},
		["Grand Widow Faerlina"] = {
		order = 2, ejid = 1602,
		28796, -- Salva de descarga de veneno
		28794, -- Lluvia de Fuego
		},
		["Maexxna"] = {
		order = 3, ejid = 1603,
		28622, -- Trampa aracnida
		54121, -- Veneno necrotico
		},
		["Noth the Plaguebringer"] = {
		order = 4, ejid = 1604,
		29213, -- Maldicion del Pesteador
		29214, -- Colera del Pesteador
		29212, -- Entorpecer
		},
		["Heigan the Unclean"] = {
		order = 5, ejid = 1605,
		29998, -- Fiebre decrepita
		29310, -- Perturbacion de hechizo
		},
		["Instructor Razuvious"] = {
		order = 7, ejid = 1607,
		55550, -- Cuchillo de sierra
		},
		["Grobbulus"] = {
		order = 11, ejid = 1611,
		28169, -- Inyecci�n mutante
		},
		["Gluth"] = {
		order = 12, ejid = 1612,
		54378, -- Herida mortal
		29306, -- Herida infectada
		},
		["Thaddius"] = {
		order = 13, ejid = 1613,
		28084, -- Carga negativa
		28059, -- Carga positiva
		},
		["Sapphiron"] = {
		order = 14, ejid = 1614,
		28522, -- Descarga de hielo
		28542, -- Drenaje de vida
		},
		["Kel'Thuzad"] = {
		order = 15, ejid = 1615,
		28410, -- Cadenas de Kel'Thuzad
		27819, -- Detonar mana
		27808, -- Explosion de Escarcha
		},
		["Trash"] = {
		order = nil, ejid = nil,
		55314, -- Estrangular
		},
	},
	[603] = { -- Ulduar
		{ id = 759, name = "Ulduar" },
		["Leviatan"] = {
		order = 1, ejid = 1637,
		},	
		["Ignis the Furnace Master"] = {
		order = 2, ejid = 1638,
		62548, -- Agostar
		62680, -- Canos de llamas
		62717, -- Olla de escoria
		},
		["Razorscale"] = {
		order = 3, ejid = 1639,
		64771, -- Fundir armadura
		},
		["XT-002"] = {
		order = 4, ejid = 1640,
		63024, -- Bomba de gravedad
		63018, -- Luz abrasadora
		},
		["The Assembly of Iron"] = {
		order = 5, ejid = 1641,
		61888, -- Poder sobrecogedor
		62269, -- Runa de Muerte
		61903, -- Golpe de fusion
		61912, -- Perturbacion estatica
		},
		["Kologarn"] = {
		order = 6, ejid = 1642,
		64290, -- Agarre petreo
		63355, -- Aplastar armadura
		62055, -- Piel quebradiza
		},
		["Auriaya"] = {
		order = 7, ejid = 1643,
		},
		["Hodir"] = {
		order = 8, ejid = 1644,
		62469, -- Congelar
		61969, -- Congelacion apresurada
		62188, -- Frio cortante
		},
		["Thorim"] = {
		order = 9, ejid = 1645,
		62042, -- Martillo de tormenta
		62130, -- Golpe desequilibrante
		62526, -- Detonacion de runa
		62470, -- Trueno ensordecedor
		62331, -- Empalar
		},
		["Freya"] = {
		order = 10, ejid = 1646,
		62532, -- Apreton de conservador
		62589, -- Furia de la naturaleza
		62861, -- Raices ferreas
		},
		["Mimiron"] = {
		order = 11, ejid = 1647,
		63666, -- Concha de Napalm
		62997, -- Explosion de plasma
		64668, -- Campo magnetico
		},
		["General Vezax"] = {
		order = 12, ejid = 1648,
		63276, -- Marca de los Ignotos
		63322, -- Vapores de saronita
		},
		["Yogg-Saron"] = {
		order = 13, ejid = 1649,
		63147, -- Ira de Sara
		63134, -- Bendicion de Sara
		63138, -- Fervor de Sara
		63830, -- Mal de la mente
		63802, -- Vinculo cerebral
		63042, -- Subyugar mente
		64152, -- Veneno drenador
		64153, -- Peste negra
		64125, -- Exprimir
		64156, -- Apatia
		64157, -- Maldicion de fatalidad
		},
		["Algalon"] = {
		order = 14, ejid = 1650,
		64412, -- Cambiar de fase
		},
		["Trash"] = {
		order = nil, ejid = nil,
		62310, -- Empalar
		63612, -- Ensena de relampagos
		63615, -- Devastar armadura
		62283, -- Raices ferreas
		63169, -- Petrificar articulaciones
		},
	},
}
